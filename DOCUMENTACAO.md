...
- Nascimento e validade usam `YYYY-MM-DD`. Na agenda, `scheduled_at` e filtros `date_from`/`date_to` usam ISO 8601 **sem fuso**, como `2030-02-05T23:30:00`; `Z` ou offsets como `-03:00` são rejeitados com 422. Envie o horário local do salão. No Compose, `SALON_TIMEZONE` configura o `TZ` de todos os salões da implantação, por padrão `America/Sao_Paulo`. Ainda não há fuso individual por salão. Caso uma instalação antiga tenha dados reais, confira horários gravados por versões que convertiam o formulário para UTC: a correção não reinterpreta registros cujo offset não foi armazenado. Os agendamentos do projeto até 08/09/2026 foram confirmados como testes; a implantação comercial começa com base nova, conforme [PRODUCTION.md](PRODUCTION.md).
